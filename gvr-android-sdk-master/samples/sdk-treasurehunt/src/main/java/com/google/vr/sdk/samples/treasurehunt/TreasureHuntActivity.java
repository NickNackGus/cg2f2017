...
public class TreasureHuntActivity extends GvrActivity implements GvrView.StereoRenderer {
  ...
  public void onDrawEye(Eye eye) {
    if (isRightEye) {
      ...
      if (frameCount == 0) {
        defectIndex = (defectIndex + 1) % 5;
      }
      if ((defectIndex == 1) && isRightEye && ((frameCount%120) == 0)) {
        hideObject();
      }
    }
    if ((defectIndex != 0) || isRightEye || ((frameCount%30) == 0)) {
      ...
      float[] originalPerspective = eye.getPerspective(Z_NEAR, Z_FAR);
      float[] perspective = new float[16];
      System.arraycopy( originalPerspective, 0, perspective, 0, 16 );
      if (defectIndex == 2) {
        // decrease fov
        float[] fovTweak = {
            1.0f, 0.0f, 0.0f, 0.0f,
        ...
        Matrix.multiplyMM(perspective, 0, fovTweak, 0, perspective, 0);
      }
      if (defectIndex == 3) {
        // increase fov
        float[] fovTweak = {
      ...
      if (defectIndex == 4) {
        ...
        Matrix.multiplyMM(perspective, 0, motionReverse, 0, perspective, 0);
      }
